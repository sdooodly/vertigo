...
export class Clouds {
  constructor(scene) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const x = Math.cos(angle)*r, y = H*0.52+Math.random()*H*0.2, z = Math.sin(angle)*r*0.3;
    ...
    this.plane = this.createAirplane();
    this.plane.position.set(0, H*0.58, 0);
    this.group.add(this.plane);
    this.dummy = new THREE.Object3D();
  }
  createAirplane(){const g=new THREE.Group();const m=new THREE.MeshStandardMaterial({color:0xeeeeee,flatShading:true});
